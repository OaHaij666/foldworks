...
import com.foldworks.moving.MovingChestRuntime;
import com.simibubi.create.api.behaviour.movement.MovementBehaviour;
import com.simibubi.create.content.contraptions.behaviour.MovementContext;
import net.minecraft.nbt.Tag;
import net.minecraft.server.level.ServerLevel;

public class FoldworksChestMovementBehaviour implements MovementBehaviour {
    ...
    private void syncMountedItemsIntoRuntime(MovementContext context, MovingChestRuntime runtime) {
        if (context.getItemStorage() instanceof FoldworksMountedItemStorage storage && storage.consumeDirty()) {
    ...
    private void syncRuntimeItemsIntoMounted(MovementContext context, MovingChestRuntime runtime) {
        if (context.getItemStorage() instanceof FoldworksMountedItemStorage storage) {
    ...
    private void syncMountedFluidsIntoRuntime(MovementContext context, MovingChestRuntime runtime) {
        if (context.getFluidStorage() instanceof FoldworksMountedFluidStorage storage && storage.consumeDirty()) {
            runtime.chest().replaceStorageFromOfflineSnapshot(
    ...
    private void syncRuntimeFluidsIntoMounted(MovementContext context, MovingChestRuntime runtime) {
        if (context.getFluidStorage() instanceof FoldworksMountedFluidStorage storage) {
