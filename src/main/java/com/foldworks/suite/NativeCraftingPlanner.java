...
import com.foldworks.api.suite.SuiteOperationTemplate;
import com.foldworks.api.suite.SuiteToolRegistry;
...
public final class NativeCraftingPlanner {
    // ABI v2 requires native-side DirectByteBuffer capacity checks and synchronized session state.
    // The previous unversioned binary is intentionally not loaded because it lacks those guarantees.
    private static final String NATIVE_ABI = "safe_v2";
    ...
    public static synchronized void tryLoad() {
        ...
        try {
            ...
            if (libPath != null) {
                ...
                loaded = true;
                Foldworks.LOGGER.info("Rust 合成规划器加载成功: {}", libPath);
            } else {
                loadFailed = true;
                Foldworks.LOGGER.warn("未找到 Rust 合成规划器原生库，将使用 Java 兜底");
            }
        } catch (UnsatisfiedLinkError | Exception e) {
            loadFailed = true;
            Foldworks.LOGGER.warn("Rust 合成规划器加载失败，将使用 Java 兜底: {}", e.getMessage());
        }
    }
    ...
    private static String nativeLibraryName() {
        ...
        return "foldworks_crafting_" + NATIVE_ABI + "_" + osPart + "_" + archPart;
    }
    ...
    private static java.nio.file.Path extractNativeLibrary(String libName) {
        ...
        for (String ext : extensions) {
            ...
            try {
                java.nio.file.Path tempDir = java.nio.file.Files.createTempDirectory("foldworks_native_");
                ...
                Foldworks.LOGGER.warn("提取原生库失败: {}", e.getMessage());
            }
        }
        return null;
    }
    ...
    private static ByteBuffer getInputBuffer(int requiredCapacity) {
        int capacity = NativeBufferSizing.capacityFor(requiredCapacity, 8192);
        if (sharedInputBuffer == null || sharedInputBuffer.capacity() < capacity) {
            sharedInputBuffer = ByteBuffer.allocateDirect(capacity);
            sharedInputBuffer.order(java.nio.ByteOrder.LITTLE_ENDIAN);
        }
        return sharedInputBuffer;
    }

    private static ByteBuffer getOutputBuffer(int requiredCapacity) {
        int capacity = NativeBufferSizing.capacityFor(requiredCapacity, 4096);
        if (sharedOutputBuffer == null || sharedOutputBuffer.capacity() < capacity) {
            sharedOutputBuffer = ByteBuffer.allocateDirect(capacity);
    ...
    public static void syncAllRecipesIfNeeded(ServerLevel level) {
        ...
        Foldworks.LOGGER.info("Rust 合成规划器：开始同步配方...");
        syncAllRecipes(level);
        Foldworks.LOGGER.info("Rust 合成规划器：同步完成, recipesSynced={}", recipesSynced);
    }
    ...
    public static synchronized void syncAllRecipes(ServerLevel level) {
        ...
        try {
            ...
            if (ok) {
                ...
                recipesSynced = true;
                Foldworks.LOGGER.info("Rust 合成规划器配方同步成功: {} 物品, {} 配方", itemIndex.size(), recipeList.size());
            } else {
                syncFailed = true;
                Foldworks.LOGGER.warn("Rust 合成规划器配方同步失败（native 返回 false）");
            }
        } catch (Throwable e) {
            syncFailed = true;
            Foldworks.LOGGER.warn("Rust 合成规划器配方同步异常: {}", e);
        }
    }
    ...
    public static List<PlanAction> plan(
            ...
            com.foldworks.blockentity.BaseChestBlockEntity chest,
        ...
        try {
            ...
            Foldworks.LOGGER.warn("Rust 合成规划器执行异常，回退 Java: {}", e);
            return null;
        }
    }
    ...
    private static List<PlanAction> doPlan(
        ...
        int orderCount = Math.min(orderContexts.size(), 255);
        int requiredInputBytes = 1 + Integer.BYTES
                + Short.BYTES + storedItems.size() * (Short.BYTES + Integer.BYTES)
                + Short.BYTES + toolPool.size() * Short.BYTES
                + 1 + orderCount * (Short.BYTES + Integer.BYTES * 3);
        ByteBuffer inputBuf = getInputBuffer(requiredInputBytes);
        inputBuf.clear();
        ...
        ByteBuffer outputBuf = getOutputBuffer(2 + orderCount * 10);
        outputBuf.clear();
        ...
        int resultCount = outputBuf.get() & 0xFF;
