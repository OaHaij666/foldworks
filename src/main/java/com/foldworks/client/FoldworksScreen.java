...
import com.foldworks.client.page.CreatePage;
import com.foldworks.client.page.ManagePage;
import com.foldworks.client.page.MigrationPage;
import com.foldworks.client.page.PermissionsPage;
import com.foldworks.client.page.ProductionStatsPage;
import com.foldworks.client.page.TabletChestPage;
import com.foldworks.client.ui.FoldworksTabletGuiTextures;
import com.foldworks.client.ui.Page;
import com.foldworks.client.ui.Router;
...
/**
 * 维度工造主界面：固定逻辑坐标系、整体缩放居中。
 *
 * 所有页面都在同一套设计尺寸里布局，窗口尺寸变化只改变整块 UI 的缩放/居中，
 * 不再让页面内部控件随窗口重新计算相对位置。
 */
public class FoldworksScreen extends Screen {
    ...
    public FoldworksScreen() { this(null); }

    public FoldworksScreen(String initialPageId) {
        super(Component.translatable("foldworks.ui.title"));
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        // 面板
        FoldworksTabletGuiTextures.shadow(g, panelX, panelY, panelW, panelH);
        FoldworksTabletGuiTextures.panel(g, panelX, panelY, panelW, panelH);
    ...
    private void renderHeader(GuiGraphics g, int mouseX, int mouseY) {
        ...
        FoldworksTabletGuiTextures.hDivider(g, panelX + 1, panelY + headerH, panelW - 2);
    ...
    private void drawCloseControl(GuiGraphics g, int x, int y, int sz, int mouseX, int mouseY) {
        ...
        if (hover) {
            FoldworksTabletGuiTextures.button(g, x, y, sz, sz, FoldworksTabletGuiTextures.ButtonState.DANGER);
            pendingTooltip = Component.translatable("foldworks.ui.close").getString();
        }
        FoldworksTabletGuiTextures.icon(g, FoldworksTabletGuiTextures.Icon.CLOSE, x + 2, y + 2, sz - 4);
    }

    private void renderSidebar(GuiGraphics g, int mouseX, int mouseY) {
        // 分隔线
        FoldworksTabletGuiTextures.vDivider(g, panelX + sidebarW, panelY + headerH + 1, panelH - headerH - 2);
        ...
        for (int i = 0; i < router.pages().size(); i++) {
            ...
            FoldworksTabletGuiTextures.button(g, ix, iy, itemSize, itemSize,
                    active ? FoldworksTabletGuiTextures.ButtonState.SELECTED
                            : hover ? FoldworksTabletGuiTextures.ButtonState.HOVER
                            : FoldworksTabletGuiTextures.ButtonState.NORMAL);
            if (active) {
                FoldworksTabletGuiTextures.hDivider(g, ix + 5, iy + itemSize - 4, itemSize - 10);
            }
    ...
    private void drawNavIcon(GuiGraphics g, Page page, int x, int y, int size) {
        FoldworksTabletGuiTextures.Icon icon = switch (page.id()) {
            case "create" -> FoldworksTabletGuiTextures.Icon.CREATE;
            case "permissions" -> FoldworksTabletGuiTextures.Icon.PERMISSIONS;
            case "production" -> FoldworksTabletGuiTextures.Icon.PRODUCTION;
            case "migration" -> FoldworksTabletGuiTextures.Icon.MIGRATION;
            case "tablet_chest" -> FoldworksTabletGuiTextures.Icon.TABLET_CHEST;
            default -> FoldworksTabletGuiTextures.Icon.MANAGE;
        };
        int iconSize = Math.max(16, Math.min(20, size - 10));
        FoldworksTabletGuiTextures.icon(g, icon, x + (size - iconSize) / 2, y + (size - iconSize) / 2, iconSize);
    }
