...
import com.foldworks.client.LocalizationUtil;
import com.foldworks.client.ui.Page;
import com.foldworks.client.ui.Theme;
import com.foldworks.client.ui.widget.UiButton;
import com.foldworks.client.ui.widget.UiDropdown;
import com.foldworks.client.ui.widget.UiSlider;
import com.foldworks.client.ui.widget.UiToggle;
import com.foldworks.config.ModConfig;
import com.foldworks.network.CreateSpaceConfigPayload;
import com.foldworks.network.CreateSpacePayload;
import com.foldworks.network.RequestDimensionBiomesPayload;
...
public class CreatePage extends Page {
    ...
    @Override public String navTitle() { return Component.translatable("foldworks.ui.nav.create").getString(); }
    ...
    private void loadData() {
        ...
        if (conn == null) return;
        dimensions.clear();
        dimensions.add(new ListEntry("random", Component.translatable("foldworks.space.dimension.random").getString()));
        try {
            for (ResourceKey<Level> key : conn.levels()) {
                ResourceLocation loc = key.location();
                if (loc.getNamespace().equals(Foldworks.MODID) && loc.getPath().startsWith("space_")) continue;
                String localized = LocalizationUtil.localizeDimension(loc.toString());
                dimensions.add(new ListEntry(loc.toString(), localized));
            }
        } catch (Exception e) {
            Foldworks.LOGGER.warn("Failed to load dimensions", e);
        }
        dataLoaded = true;
        widgetsBuilt = false;
    }
    ...
    private void buildWidgets() {
        ...
        biomeDropdown = new UiDropdown(new ArrayList<>(List.of(
                Component.translatable("foldworks.space.biome.random").getString())), 0);
        biomeIds.clear();
        biomeIds.add("random");

        mobToggle = new UiToggle(Component.translatable("foldworks.space.mob_spawning").getString(), false);
        structToggle = new UiToggle(Component.translatable("foldworks.space.structures").getString(), false);
        amplitudeToggle = new UiToggle(Component.translatable("foldworks.space.amplitude_enable").getString(), false);
        amplitudeSlider = new UiSlider(Component.translatable("foldworks.space.amplitude").getString(), 0.4f);

        createBtn = new UiButton(Component.translatable("foldworks.space.create_and_enter").getString(), UiButton.Variant.PRIMARY)
                .onClick(this::createSpace);
        cancelBtn = new UiButton(Component.translatable("foldworks.space.cancel").getString(), UiButton.Variant.GHOST)
    ...
    private EditBox makeSizeBox(String def) {
        int max = maxSize();
        EditBox box = new EditBox(font, 0, 0, 60, 16, Component.translatable("foldworks.space.size.placeholder"));
    ...
    private void refreshBiomeList() {
        ...
        biomeIds.add("random");
        labels.add(Component.translatable("foldworks.space.biome.random").getString());
    ...
    public void render(GuiGraphics g, int mouseX, int mouseY, float partialTick) {
        ...
        // 空间类型：平坦 / 自然 / 无限
        cy = section(g, formX, cy, Component.translatable("foldworks.space.terrain").getString(), true);
        ...
        if (natural) {
            ...
            cy += 24;
            g.drawString(font, Theme.styled(Component.translatable("foldworks.space.amplitude.warning").getString()), formX + 8, cy + 2, Theme.TEXT_FAINT, false);
            cy += 14;
        ...
        // 尺寸
        cy = section(g, formX, cy, Component.translatable("foldworks.space.size").getString(), !infinite);
        layoutSize(g, formX, cy, formW, mouseX, mouseY, !infinite);
        cy += 28;

        // 继承维度（始终可用）
        cy = section(g, formX, cy, Component.translatable("foldworks.space.dimension").getString(), true);
        ...
        cy = section(g, formX, cy, Component.translatable("foldworks.space.biome").getString(), biomeEnabled);
        if (biomeEnabled) {
            ...
            String hint = infinite
                    ? Component.translatable("foldworks.space.biome.inherited").getString()
                    : Component.translatable("foldworks.space.biome.pick_dim_first").getString();
        ...
        String createLabel = status.isEmpty()
                ? Component.translatable("foldworks.space.create_and_enter").getString()
                : status;
    ...
    private void layoutTerrain(GuiGraphics g, int x, int y, int w, int mouseX, int mouseY, boolean enabled) {
        ...
        for (int i = 0; i < TERRAIN_MODES.length; i++) {
            ...
            String label = switch (TERRAIN_MODES[i]) {
                case FLAT -> Component.translatable("foldworks.space.terrain.superflat").getString();
                case NATURAL -> Component.translatable("foldworks.space.terrain.natural").getString();
                case INFINITE -> Component.translatable("foldworks.space.infinite_tag").getString();
            };
    ...
    private void layoutSize(GuiGraphics g, int x, int y, int w, int mouseX, int mouseY, boolean enabled) {
        ...
        drawField(g, x, y, half, Component.translatable("foldworks.space.size.width").getString(), widthInput, enabled);
        drawField(g, x + half + Theme.GAP, y, half, Component.translatable("foldworks.space.size.depth").getString(), depthInput, enabled);
    }
    ...
    private void createSpace() {
        ...
        if (!infinite) {
            ...
            if (wv < min || wv > max || dv < min || dv > max) {
                status = Component.translatable("foldworks.space.size.invalid").getString();
                return;
            }
        }
        status = Component.translatable("foldworks.space.creating").getString();
