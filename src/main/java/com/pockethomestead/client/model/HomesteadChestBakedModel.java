...
public class HomesteadChestBakedModel extends BakedModelWrapper<BakedModel> {
    ...
    public HomesteadChestBakedModel(BakedModel originalModel, Function<Material, TextureAtlasSprite> textureGetter) {
        super(originalModel);
    ...
    private List<BakedQuad> moduleQuadsFor(@Nullable BlockState state, ModelData extraData) {
        ...
        Direction front = state.hasProperty(AbstractHomesteadBlock.FACING)
                ? state.getValue(AbstractHomesteadBlock.FACING)
                : Direction.NORTH;
    ...
    private static TextureAtlasSprite sprite(Function<Material, TextureAtlasSprite> textureGetter, String texture) {
        ResourceLocation location = ResourceLocation.fromNamespaceAndPath(PocketHomestead.MODID, "block/" + texture);
