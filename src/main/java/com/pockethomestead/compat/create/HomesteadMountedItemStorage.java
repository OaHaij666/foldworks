...
public class HomesteadMountedItemStorage extends MountedItemStorage {
    public static final MapCodec<HomesteadMountedItemStorage> CODEC = RecordCodecBuilder.mapCodec(instance -> instance.group(
            Codec.INT.fieldOf("capacity").forGetter(storage -> storage.maxItemCapacity),
            Entry.CODEC.listOf().fieldOf("items").forGetter(HomesteadMountedItemStorage::entriesForCodec)
    ).apply(instance, HomesteadMountedItemStorage::new));

    private final List<StoredItemStack> items = new ArrayList<>();
    private int maxItemCapacity;
    private boolean dirty;

    HomesteadMountedItemStorage(MountedItemStorageType<?> type, BaseChestBlockEntity chest) {
        super(type);
        this.maxItemCapacity = chest.getMaxItemCapacity();
        replaceItems(chest.getStoredItems());
    }

    private HomesteadMountedItemStorage(int maxItemCapacity, List<Entry> entries) {
