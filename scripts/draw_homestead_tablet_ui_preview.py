...
from generate_homestead_tablet_gui_atlas import OUT as ATLAS_PATH
from generate_homestead_tablet_gui_atlas import draw_atlas


ROOT = Path(__file__).resolve().parents[1]
OUT = ROOT / "docs" / "client-ui" / "previews" / "homestead-tablet-ui-preview-v1.png"
...
def font(size: int) -> ImageFont.FreeTypeFont | ImageFont.ImageFont:
    for path in (
        ROOT / "src/main/resources/assets/pockethomestead/font/notosanssc-regular.ttf",
...
def base_screen(active: str) -> Image.Image:
    ...
    text(d, "尘歌玉盘", px + 12, py + 9, INK, F11)
...
def draw_manage(img: Image.Image):
    ...
    pad = 12
    text(d, "选择要进入的家园", x + pad, y + pad, MUTED, F10)
    fy = y + pad + 16
    ...
    button(img, d, x + pad, y + h - 36, w - pad * 2, 28, "新建家园", "button_primary", BLUE_DEEP, "create")
